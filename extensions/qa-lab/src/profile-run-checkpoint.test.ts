...
function evidence(scenarioId: string, status: QaEvidenceStatus = "pass"): QaEvidenceSummaryJson {
  return validateQaEvidenceSummaryJson({
    ...
    entries: [
      {
        test: { kind: "flow", id: scenarioId, title: scenarioId },
        coverage: [],
        refs: [],
        result: { status },
      },
...
describe("QA profile run checkpoint", () => {
  ...
  it("serializes atomic starts and forbids terminal regression", async () => {
    ...
    await checkpoint.reporter.complete({
      ...
      evidence: evidence(firstScenario.id),
      result: "pass",
    });
  ...
  it("rejects missing or tampered durable evidence refs", async () => {
    const { checkpoint, outputDir } = await createCheckpoint();
    await checkpoint.reporter.start(firstScenario.id);
    await expect(
      checkpoint.reporter.complete({
        scenarioId: firstScenario.id,
        evidence: evidence("another-scenario"),
        result: "pass",
      }),
    ).rejects.toThrow(`does not contain scenario ${firstScenario.id}`);
